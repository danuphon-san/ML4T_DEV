...
SUMMARY_PATH = OUTPUT_DIR / "sp20_seed_backtest_summary.json"
REPORT_PATH = OUTPUT_DIR / "sp20_seed_backtest_report.md"


class WeeklyLongShort3x3(LongShortStrategy):
...
def build_prices(test_frame: pl.DataFrame) -> pl.DataFrame:
    ...
    ) if {"open", "high", "low", "close", "volume"}.issubset(set(test_frame.columns)) else test_frame.select(
        [
            "timestamp",
            pl.col("symbol").alias("asset"),
...
def main() -> None:
    ...
    winner_text = (
        f"`{winner['name']}` is the strongest backtest by Sharpe/return in this first pass. "
